// PropPopulator.cs
// Drop-in prop placement with grouping + trap avoidance + simple tasteful rules.
...
public sealed class PropPopulator : MonoBehaviour
{
    ...
    [Header("Global Taste (good defaults)")]
    [Range(0, 3)] public int doorAisleDepth = 2;       // clear path in from each door
    [Range(0, 2)] public int wallClamp = 1;            // how tight wall props hug walls
    [Range(0.5f, 3f)] public float densityPer64 = 1.4f; // interior density per 64 tiles
    ...
    /// Call this AFTER traps are built, so we can avoid them.
        ...
        IReadOnlyCollection<Vector2Int> traps = null)
    {
        ...
        if (traps != null) foreach (var c in traps) trapCells.Add(c);
        ...
        foreach (var room in rooms)
        {
            ...
            // Pick biome rules by dominant interior kind (robust — center may be corridor).
            ...
            // Corners: 0–2 tasteful highlights (never block doors) 
            ...
            // Walls: evenly spaced lines, slight jitter, hugging walls
            ...
            //  Interior: blue-noise sprinkle, density by area 
            ...
            // enforce budget by trimming slots (we’ll still respect per-prop min/max inside)
            TrimSlotsDeterministic(interiorSlots, interiorBudget * 6, rng); // generous candidate pool
    ...
    private (List<SimpleProp> corner, List<SimpleProp> wall, List<SimpleProp> interior) PickBiomeLists(string prefix)
    {
        ...
        return (entryCorner, entryWall, entryInterior); // default & entry
    }
    ...
    private static List<Vector2Int> BestCorners(RoomInfo info, int maxCount)
    {
        ...
        for (int i = 0; i < take; i++) list.Add(ranked[i].c);
        return list;
    }
    ...
    private HashSet<Vector2Int> BuildAisles(DungeonMapIndex index, Room room, int depth)
    {
        ...
        foreach (var e in room.Info.Entrances)
        {
            ...
            {
                var n = e + d;
                if (index.CorridorId[n.x, n.y] >= 0) { corridor = n; break; }
            }
    ...
    // ---------- Placement ----------
    private void PlaceFromBucket(
        DungeonGrid grid,
        Room room,
        List<Vector2Int> slots,
        List<SimpleProp> props,
        DungeonMapIndex index,
        bool[,] carpetMask,
        ...
        System.Random rng)
    {
        ...
        // One candidate list per bucket; shuffle deterministically once.
        ...
        foreach (var rule in props)
        {
            ...
            // Requested items for this rule, capped by room density.
    ...
    private List<Vector2Int> BuildCandidates(
        List<Vector2Int> baseSlots,
        RoomInfo info,
        bool[,] carpet,
        HashSet<Vector2Int> forbidden)
    {
        ...
        bool[,] carpetMask)
    {
        ...
        foreach (var c in candidates)
        {
            ...
            if (rule.separation > 0)
            {
                ...
                {
                    if (Mathf.Max(Mathf.Abs(a.x - c.x), Mathf.Abs(a.y - c.y)) < rule.separation) { tooClose = true; break; }
                }
        ...
        Room room)
    {
        ...
        foreach (var center in candidates)
        {
            ...
            {
                if (Cheb(center, cc) < rule.minDistanceBetweenGroups) { clash = true; break; }
            }
            if (clash) continue;
            ...
            while (groupSize > 1 && placed < allowed && tries < 48)
            {
                ...
                // keep satellites loosely apart
                bool tooClose = false;
                foreach (var cc in centers)
                {
                    if (Cheb(p, cc) < rule.separation) { tooClose = true; break; }
                }
                if (tooClose) continue;
